...
namespace CalApp.Shared.JPower
{
    public class JPowerDevice : ObservableObject, IJPowerDevice
    {
        public JPowerDevice(BleDevice bleDevice)
        {
            ...
            calService          = GetService(JPowerBleUUIDs.JPOWER_CAL_SRV_UUID);
            adcRawChar          = GetCharacteristic(calService, JPowerBleUUIDs.JPOWER_CAL_SRV_ADC_RAW_UUID);
            zeroChar            = GetCharacteristic(calService, JPowerBleUUIDs.JPOWER_CAL_SRV_ZERO_UUID);
            pullCalReqChar      = GetCharacteristic(calService, JPowerBleUUIDs.JPOWER_CAL_SRV_PULL_CAL_REQ_UUID);
            pullCalChar         = GetCharacteristic(calService, JPowerBleUUIDs.JPOWER_CAL_SRV_PULL_CAL_UUID);
            pushCalChar         = GetCharacteristic(calService, JPowerBleUUIDs.JPOWER_CAL_SRV_PUSH_CAL_UUID);

            stateService        = GetService(JPowerBleUUIDs.JPOWER_JP_STATE_SRV_UUID);
            stateRequestChar    = GetCharacteristic(stateService, JPowerBleUUIDs.JPOWER_JP_STATE_SRV_STATE_REQUEST_UUID);
            currentStateChar    = GetCharacteristic(stateService, JPowerBleUUIDs.JPOWER_JP_STATE_SRV_CURRENT_STATE_UUID);
